    ...
    <div class="row">
        <div class="col-md-8 column">
            <form class="form-inline">
                <input type="text"  id="emp_search" class="form-control" placeholder="请输入员工的关键词或id">
                        <%-- 查找按钮 --%>
                <button class="btn btn-primary glyphicon glyphicon-search"
                                type="button" id="emp_search_btn">Search</button>
            </form>
        </div>
        <div class="col-md-4">
            <button type="button" class="btn btn-primary" id="emp_add_modal_btn">update</button>
            <button type="button" class="btn btn-danger" id="emp_delete_batch_btn">delete</button>
        </div>
    </div>
...
<script type="text/javascript">
    ...
    $("#emp_update_btn").click(function () {
        ...
        $.ajax({
            ...
            type: "PUT",
            data: $("#empsUpdateModal form").serialize(),// + "&_method=PUT",
    ...
    //由于此类方法是绑定在页面成功创建之后才有的按钮上的，所以更新此类方法后需要重启服务器，启动功能
    //单个删除
    $(document).on("click", ".delete_btn", function () {
        ...
        if (confirm("Do you really want to delete【No." + empId + " " + empName + "】?")) {
            $.ajax({
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                $.each(result.extend.depts, function () {
                    ...
                    //将option标签添加到empsAddModal下的select标签中
    ...
    $("#emp_search_btn").click(function () {
        ...
        var searchContent = $("#emp_search").val();
    ...
    function search_emps_table(result) {
        ...
        var empSearched = result.extend.pageInfo.list;
        ...
        if(empSearched.length == 0){
            ...
            //遍历所有查询到的员工
            $.each(empSearched, function (index, item) {
                //在员工数据的最左边加上多选框
    ...
    function build_search_page_info(result) {
        ...
        $("#page_info_area").empty();
        //显示共几条记录
        $("#page_info_area").append("已查询到" + result.extend.pageInfo.total +"条记录。");
